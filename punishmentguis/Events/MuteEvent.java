...
public class MuteEvent {
    ...
    public void onClick(InventoryClickEvent e) {
        if (e.getView().getTitle().contains("&7Mute Punishments")) {
            e.setCancelled(true);
            ...
            if (e.getCurrentItem() != null) {
                if (e.getSlot() == 11) {
                    p.closeInventory();
                    main.getReasonList().put(p, "15m");
                    p.sendMessage(Utils.toColor(main.getConfig().getString("mute-reason-message").replaceAll("\\{player}", target.getName())));
                }
                if (e.getSlot() == 12) {
                    p.closeInventory();
                    main.getReasonList().put(p, "30m");
                    p.sendMessage(Utils.toColor(main.getConfig().getString("mute-reason-message").replaceAll("\\{player}", target.getName())));
                }
                if (e.getSlot() == 13) {
                    p.closeInventory();
    ...
    public void onChat(AsyncPlayerChatEvent e) {
        ...
        if (main.getReasonList().containsKey(p)) {
            ...
            Bukkit.dispatchCommand(p, "tempmute " + target.getName() + " " + main.getReasonList().get(p) + "  " + e.getMessage());
